...
export function BlogGrid({ posts, page, categorySlug }: { posts: BlogPost[]; page: number; categorySlug?: string }) {
  ...
  const prefix = categorySlug ? `/blog/category/${categorySlug}` : "";
  const basePath = prefix ? `${prefix}/page` : "/blog/page";
  const firstPage = prefix || "/blog";
    ...
    <div className="flex flex-1 flex-col items-center justify-start gap-16">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-3">
          {page > 1 && (
            <Link
              href={page === 2 ? firstPage : `${basePath}/${page - 1}`}
